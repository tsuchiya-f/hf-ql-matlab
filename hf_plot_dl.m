...
%     ex) $ nc localhost 7902 > hf_mode0.tlm 
% (4) Run this program and select "hf_mode0.tlm" as an input data
...
[file,dir] = uigetfile('C:\temp\juice\data\*.tlm');
...
while true
    ...
    %Skip HF header (the first 24Byte)
    HF_Hdr = uint8(fread(r,24));
...
%        fprintf('%d %04X %04X %d\n', loop, header(1), header(2), r.BytesAvailable);
        fprintf('%d\n', loop);
        rawData = vertcat(rawData, swapbytes(typecast(uint8(fread(r,2048)),'int16')));   %data
